            ...
            {pending.map(t => (
              <div key={t.id} className="px-6 py-5 flex items-start justify-between gap-6 flex-wrap md:flex-nowrap">
                <div className="flex items-start gap-4">
                  {t.avatar ? (
                    <img src={encodeURI(decodeURI())} alt={t.name} className="w-12 h-12 rounded-full object-cover shrink-0 mt-1" />
                  ) : (
                ...
                <tr key={t.id} className={`hover:bg-slate-50/50 transition-colors ${t.isSuspended ? "bg-red-50/30" : ""}`}>
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-3">
                      {t.avatar ? (
                        <img src={encodeURI(decodeURI())} alt={t.name} className="w-9 h-9 rounded-full object-cover shrink-0" />
                      ) : (
