...
export default async function TherapistDashboardPage() {
  ...
  const [
    ...
    prisma.appointment.count({ where: { therapistId: userId, status: { in: ["CONFIRMED", "PENDING"] }, scheduledAt: { gte: new Date(now.getTime() - 2 * 60 * 60 * 1000) } } }),
    prisma.appointment.findFirst({ where: { therapistId: userId, status: "IN_PROGRESS" }, include: { patient: true } }),
    prisma.appointment.findMany({ where: { therapistId: userId, status: { in: ["PENDING", "CONFIRMED"] }, scheduledAt: { gte: new Date(now.getTime() - 2 * 60 * 60 * 1000) } }, include: { patient: true }, orderBy: { scheduledAt: "asc" }, take: 5 }),
